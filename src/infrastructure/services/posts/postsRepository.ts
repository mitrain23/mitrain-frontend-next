...
export class PostsRepository {
    static getAllPost = async (pageNumber: number): Promise<Post[]> => {
        const response = await fetch(`${API_BASE_URL}/api/allPosts?page=${pageNumber}&pageSize=10`);
    ...
    static getPostByAuthor = async (id: string | null): Promise<GetPostByAuthorResponse> => {
        const token = Cookies.get('token');
        console.log(token);
        console.log(id);
